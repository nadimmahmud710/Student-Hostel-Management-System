<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        $query = "SELECT * FROM user WHERE (email = '$email_entered' AND password = '$password_entered')";
        $result = mysqli_query($db, $query);

        if(mysqli_num_rows($result) == 1)
        {
            $row = mysqli_fetch_array($result);
            if($row['email'] == $email_entered && $row['password'] == $password_entered)
            {
                $_SESSION['id']=$row['id'];
                $_SESSION['name']=$row['name'];
                header('location: user/user_home_page.php');
            }
            else
            {
